...
def test_build_node_ingest_query_no_overwrite():
    query = _build_node_ingest_query("Database", False, ["name", "description", "service", "platform"])
    assert query == """
UNWIND $rows as row
...
def test_build_node_ingest_query_overwrite():
    query = _build_node_ingest_query("Database", True, ["name", "description", "service", "platform"])
    assert query == """
UNWIND $rows as row
MERGE (n:Database {id: row.id})
SET n.name = row.name,
...
def test_build_relationship_ingest_query_no_overwrite():
    query = _build_relationship_ingest_query("HAS_SCHEMA", "Database", "Schema", "database_id", "schema_id", False, ["name", "description"])
    assert query == """
UNWIND $rows as row
...
def test_build_relationship_ingest_query_overwrite():
    query = _build_relationship_ingest_query("HAS_SCHEMA", "Database", "Schema", "database_id", "schema_id", True, ["name", "description"])
    assert query == """
UNWIND $rows as row
MATCH (n1:Database {id: row.database_id})
MATCH (n2:Schema {id: row.schema_id})
...
def test_build_relationship_ingest_query_no_properties():
    query = _build_relationship_ingest_query("HAS_SCHEMA", "Database", "Schema", "database_id", "schema_id", False, [])
    assert query == """
UNWIND $rows as row
MATCH (n1:Database {id: row.database_id})
MATCH (n2:Schema {id: row.schema_id})
