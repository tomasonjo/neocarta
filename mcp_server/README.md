...
- **Input:** `query: str` — a natural language question or keyword
...
- **Input:**
  - `query: str` — a natural language question or keyword
...
- **Use when:** the query maps to a domain or subject area rather than a specific column — e.g. `"sales performance"` or `"user activity"`.
...
- **Use when:** a complete picture of the schema is needed — e.g. for schema registration, evaluation baselines, or debugging. **This is an expensive query** and should not be used in hot paths.
