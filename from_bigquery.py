...
def get_column_references_info(
    client: bigquery.Client, project_id: str, dataset_id: str
) -> pd.DataFrame:
    return client.query(f"""
SELECT 
...
def get_table_nodes(table_info: pd.DataFrame) -> list[Table]:
    return [
        Table(
            id=info_row.table_catalog
            + "."
            + info_row.table_schema
            + "."
            + info_row.table_name,
...
def get_references_relationships(
...
def load_contains_table_relationships(
    contains_table_relationships: list[ContainsTable], neo4j_driver: Driver
) -> dict:
    _, summary, _ = neo4j_driver.execute_query(
        query_="""
    UNWIND $rows as row
...
def load_has_column_relationships(
    has_column_relationships: list[HasColumn], neo4j_driver: Driver
) -> dict:
    _, summary, _ = neo4j_driver.execute_query(
        query_="""
    UNWIND $rows as row
...
def load_references_relationships(
    references_relationships: list[References], neo4j_driver: Driver
) -> dict:
    _, summary, _ = neo4j_driver.execute_query(
        query_="""
    UNWIND $rows as row
...
if __name__ == "__main__":
    ...
    column_references_info = get_column_references_info(
